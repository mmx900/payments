...
export class FakeTossPaymentsController {
   ...
   * 서버가 현재의 `payments.at` 을 통하여 해당 결제 정보를 확인하고, {@link confirm} 를
   ...
   * 직접 호출하는 경우, 토스 페이먼츠의 결제 창을 이용하여 별도의 {@link confirm} 가
   ...
   * 페이먼츠의 결제 창을 이용한 카드 결제의 경우, 별도의 {@link confirm} 가 필요한
   * 상황을 시뮬레이션 할 수 있다.
   *
   ...
   * 확정하지 않는다. `payments.confirm` 는 바로 이러한 상황에서, 해당 결제를 승인해주는
   * 함수이다.
   *
